<?php
// app_bitdefender_sync_client_v2.php - Versão melhorada com debug
session_start();
...
function syncSingleClient($pdo, $client, $accessUrl) {
    ...
    try {
        ...
        logClientSync($pdo, $client['id'], 'success', "Cliente sincronizado em {$duration}s", [
            ...
            'api_response_fields' => array_keys($licenseInfo)
        ]);
        ...
        return [
            ...
            'debug' => [
                'api_fields_received' => array_keys($licenseInfo),
                'api_response' => $licenseInfo
            ]
        ];
...
function logClientSync($pdo, $clientId, $status, $message, $details = []) {
    try {
        ...
        // Ignorar erro de log para não interromper sincronização
