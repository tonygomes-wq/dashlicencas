...
interface GmailDetailModalProps {
    ...
    licenses: GmailLicenseWithClient[];
    onLicenseUpdate: (id: number, data: Partial<GmailLicense>) => Promise<void>;
    onLicenseRemove: (id: number) => Promise<void>;
    onAddLicense: (clientId: string, licenseData: Omit<GmailLicense, 'id' | 'clientId' | 'renewalStatus'>) => Promise<void>;
    onBulkImport: (clientId: string, licenses: Omit<GmailLicense, 'id' | 'clientId' | 'renewalStatus'>[]) => Promise<void>;
    isAdmin: boolean;
}
...
const initialNewLicenseState: Omit<GmailLicense, 'id' | 'clientId' | 'renewalStatus'> = {
    username: '',
    email: '',
    password: '',
    licenseType: '',
...
const GmailDetailModal: React.FC<GmailDetailModalProps> = ({ isOpen, onClose, client, licenses, onLicenseUpdate, onLicenseRemove, onAddLicense, onBulkImport, isAdmin }) => {
    const [editingLicenseId, setEditingLicenseId] = useState<number | null>(null);
    const [tempEditData, setTempEditData] = useState<Partial<GmailLicense>>({});
    ...
    const handleEditClick = (license: GmailLicense) => {
    ...
    const handleSaveEdit = async (license: GmailLicense) => {
        if (!isAdmin) {
            toast.error("Você não tem permissão para editar este registro.");
            return;
        }

        const updates: Partial<GmailLicense> = {};
    ...
    const handleDeleteClick = async (license: GmailLicense) => {
        if (!isAdmin) {
    ...
    const handleBulkImport = async (licenses: Omit<GmailLicense, 'id' | 'clientId' | 'renewalStatus'>[]) => {
        if (!client) return;
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl max-w-full w-full mx-4 lg:mx-8 transform transition-all flex flex-col max-h-[90vh]">
                    <div className="flex justify-between items-center p-4 border-b dark:border-gray-700">
                        <h2 className="text-xl font-bold text-gray-800 dark:text-white">
                            Licenças do Cliente: {client.clientName} (GMAIL)
                        </h2>
...
export default GmailDetailModal;