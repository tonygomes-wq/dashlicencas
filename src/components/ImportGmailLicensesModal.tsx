...
import { GmailLicense } from '../types';

interface ImportGmailLicensesModalProps {
  isOpen: boolean;
  onClose: () => void;
  onImport: (licenses: Omit<GmailLicense, 'id' | 'clientId' | 'renewalStatus'>[]) => Promise<void>;
}

// Campos obrigatórios para uma licença GMAIL
const REQUIRED_FIELDS = ['username', 'email', 'licenseType'];
const OPTIONAL_FIELDS = ['password'];

const ImportGmailLicensesModal: React.FC<ImportGmailLicensesModalProps> = ({ isOpen, onClose, onImport }) => {
  ...
  const handleImportData = useCallback(async () => {
    ...
    try {
      ...
      reader.onload = async (e) => {
        ...
        // defval: '' garante que células vazias sejam strings vazias em vez de undefined
        const rawJson: any[] = XLSX.utils.sheet_to_json(worksheet, { defval: '' });

        console.log('Dados brutos da planilha:', rawJson);
        console.log('Mapeamento de colunas:', columnMapping);

        const licensesToImport: Omit<GmailLicense, 'id' | 'clientId' | 'renewalStatus'>[] = [];
        ...
        console.log('Headers mapeados:', { usernameHeader, emailHeader, licenseTypeHeader, passwordHeader });

        rawJson.forEach((row, index) => {
          // Tenta encontrar o email de várias formas (case-insensitive)
          let email = '';
          
          // Primeiro tenta o header exato
          if (row[emailHeader] !== undefined) {
            email = row[emailHeader]?.toString().trim() || '';
          } else {
            // Se não encontrar, procura por variações do header (case-insensitive)
            const keys = Object.keys(row);
            const emailKey = keys.find(k => k.toLowerCase().trim() === emailHeader.toLowerCase().trim());
            if (emailKey) {
              email = row[emailKey]?.toString().trim() || '';
            }
          }

          // Também tenta encontrar username e licenseType de forma flexível
          let username = '';
          ...
          // Username
          if (row[usernameHeader] !== undefined) {
            username = row[usernameHeader]?.toString().trim() || '';
          } else {
            const usernameKey = keys.find(k => k.toLowerCase().trim() === usernameHeader.toLowerCase().trim());
            if (usernameKey) username = row[usernameKey]?.toString().trim() || '';
          }

          // License Type
          if (row[licenseTypeHeader] !== undefined) {
            licenseType = row[licenseTypeHeader]?.toString().trim() || 'Standard';
          } else {
            const licenseKey = keys.find(k => k.toLowerCase().trim() === licenseTypeHeader.toLowerCase().trim());
            if (licenseKey) licenseType = row[licenseKey]?.toString().trim() || 'Standard';
          }

          // Password (opcional)
          if (passwordHeader) {
            if (row[passwordHeader] !== undefined) {
              password = row[passwordHeader]?.toString().trim() || null;
            } else {
              const passwordKey = keys.find(k => k.toLowerCase().trim() === passwordHeader.toLowerCase().trim());
              if (passwordKey) password = row[passwordKey]?.toString().trim() || null;
            }
          }

          console.log(`Linha ${index + 1}:`, { email, username, licenseType, password });
        ...
        console.log('Total de licenças válidas:', licensesToImport.length);

        if (licensesToImport.length === 0) {
          toast.error('Nenhuma licença válida encontrada no arquivo. Verifique se a coluna de email contém emails válidos.', { id: toastId });
          setIsProcessing(false);
          return;
        }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl max-w-3xl w-full transform transition-all flex flex-col max-h-[90vh]">
        <div className="flex justify-between items-center p-4 border-b dark:border-gray-700">
          <h2 className="text-xl font-bold text-gray-800 dark:text-white">Importar Licenças GMAIL</h2>
...
export default ImportGmailLicensesModal;