...
import { O365Client, O365License } from '../types';
...
interface AddO365ClientModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (clientData: Omit<O365Client, 'id'>, licenses: Omit<O365License, 'id' | 'clientId' | 'renewalStatus'>[]) => Promise<void>;
}
...
const AddO365ClientModal: React.FC<AddO365ClientModalProps> = ({ isOpen, onClose, onSave }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const clientData: Omit<O365Client, 'id'> = { clientName, contactEmail: contactEmail || null };
    const licensesData: Omit<O365License, 'id' | 'clientId' | 'renewalStatus'>[] = licenses.map(l => ({
        username: l.username,
        email: l.email,
                    ...
                    {licenses.map((license, index) => (
                        <div key={index} className="p-3 border rounded-md bg-gray-50 dark:bg-gray-700/50 flex flex-wrap gap-3 items-end">
                            ...
                            <div className="flex-1 min-w-[150px]">
                                ...
                                <input type="text" name="licenseType" placeholder="Ex: E3, Business Basic" value={license.licenseType} onChange={(e) => handleLicenseChange(index, e)} required className="w-full px-3 py-2 border rounded bg-white dark:bg-gray-600 text-gray-900 dark:text-white text-sm" />
                            </div>
                            <div className="flex-1 min-w-[150px]">
...
export default AddO365ClientModal;