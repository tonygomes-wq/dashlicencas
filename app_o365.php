...
switch ($method) {
    case 'GET':
        $filter = getClientFilter('o365');
        if ($type === 'clients') {
            $sql = 'SELECT * FROM o365_clients';
            if ($filter) {
            ...
            $pdo->exec("DELETE t1 FROM o365_licenses t1
                       INNER JOIN o365_licenses t2 
                       WHERE t1.id < t2.id 
                       AND t1.email = t2.email 
                       AND t1.client_id = t2.client_id");

            $sql = 'SELECT * FROM o365_licenses';
            if ($filter) {
                $placeholders = implode(',', array_fill(0, count($filter), '?'));
    ...
    case 'POST':
        ...
        if ($action === 'bulk_create' && $type === 'licenses') {
            ...
            try {
                $insertedLicenses = [];
                $sql = "INSERT INTO o365_licenses (client_id, user_id, username, email, password, license_type, renewal_status) VALUES (?, ?, ?, ?, ?, ?, ?)";
                $stmt = $pdo->prepare($sql);
                ...
                foreach ($licenses as $license) {
                    ...
                    $selectStmt = $pdo->prepare('SELECT * FROM o365_licenses WHERE id = ?');
        ...
        if ($action === 'create_with_licenses' && $type === 'clients') {
            ...
            try {
                ...
                $sql = "INSERT INTO o365_clients (id, user_id, client_name, contact_email) VALUES (?, ?, ?, ?)";
                $stmt = $pdo->prepare($sql);
                $stmt->execute([
                    $id,
                    $user_id,
                ...
                $insertedLicenses = [];
                if (!empty($licenses)) {
                    $licenseSql = "INSERT INTO o365_licenses (client_id, user_id, username, email, password, license_type, renewal_status) VALUES (?, ?, ?, ?, ?, ?, ?)";
                ...
                $stmt = $pdo->prepare('SELECT * FROM o365_clients WHERE id = ?');
                $stmt->execute([$id]);
                $client = $stmt->fetch();
        ...
        if ($type === 'clients') {
            $id = $data['id'] ?? bin2hex(random_bytes(16)); // UUID-like
            $sql = "INSERT INTO o365_clients (id, user_id, client_name, contact_email) VALUES (?, ?, ?, ?)";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([
                $id,
            ...
            $new_id = $pdo->lastInsertId();
            $stmt = $pdo->prepare('SELECT * FROM o365_licenses WHERE id = ?');
            $stmt->execute([$new_id]);
            echo json_encode($stmt->fetch());
        }
        break;
    ...
    case 'PUT':
        ...
        $params[] = $id;
        $table = ($type === 'clients') ? 'o365_clients' : 'o365_licenses';
